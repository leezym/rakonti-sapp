...
function StepOne({ data, setData, formData, setFormData, id_usuario }) {
  const [generosUsuario, setGenerosUsuario] = useState([]);
  const [generos, setGeneros] = useState([]);

  useEffect(() => {
    axios.get(`http://localhost:5001/rakonti/usuario-generos/${id_usuario}`)
    .then(res => {
        setGenerosUsuario(res.data);
      })
      .catch(err => {
        console.error('Error al cargar los géneros del usuario:', err);
      });
  }, []);
  ...
  return (
    <>
      <FormContainer>
        <p style={{ color: '#43474f' }}>El <b>Género</b> es la categoría narrativa que define el estilo, tono y tipo de conflicto de la historia. Basado en tus intereses te recomendamos los siguientes géneros:
          <b> {generosUsuario.map(genero => genero.nombre).join(', ')}.</b>
        </p>
...
function StepTwo({ data, setData, formData, setFormData }) {
  ...
  return (
    <>
      <FormContainer>
        <p style={{ color: '#43474f' }}>La <b>Trama</b> pdte texto:</p>
        <p style={{ color: '#43474f' }}><b>Elige la trama que deseas usar para tu historia</b></p>
...
function StepThree({ data, setData, formData, setFormData }) {
  ...
  return (
    <>
      <FormContainer>
        <p style={{ color: '#43474f' }}>El <b>Objeto del deseo</b> pdte texto:</p>
        <p style={{ color: '#43474f' }}><b>Elige el objeto del deseo que deseas usar para tu historia</b></p>
...
function StepFour({ data, setData, formData, setFormData }) {
  ...
  return (
    <>
      <FormContainer>
        <p style={{ color: '#43474f' }}>El <b>Tiempo y Espacio</b> pdte texto:</p>
        <p style={{ color: '#43474f' }}><b>Elige la trama que deseas usar para tu historia</b></p>
...
function StepFive({ data, setData, formData, setFormData }) {
  return (
    <>
...
function RFeaturesView() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Editar historia
    if(id_historia && id_historia === formData.id_historia)
    {
      try {
  ...
  return <>
        ...
        <RightColumn>
          ...
          <StepsContainer>
            ...
            <Step active={step === 4} step={4} padding="20px 40px 20px 40px">
              ...
              <br />{data.tiempo_espacio ? data.tiempo_espacio : "Ninguno"}          
            </Step>
    ...
    <Container>
      {step === 1 && <StepOne data={data} setData={setData} formData={formData} setFormData={setFormData} id_usuario={id_usuario} />}
      ...
      {step === 5 && <StepFive data={data} setData={setData} formData={formData} setFormData={setFormData} />}
      {step === 6 && <StepSix/>}
